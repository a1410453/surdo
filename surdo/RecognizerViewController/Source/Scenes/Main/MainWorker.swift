...
class DefaultMainWorker: MainWorker {
    ...
    private var model: KSLClassifier?
    private var result: KSLClassifierOutput?
    ...
    private var resultHandler: ((_ value: String?, 
                                 _ confidence: VNConfidence?,
                                 _ error: CustomError?) -> Void)?
    ...
    // MARK: - Methods
    init() {
        model = try? KSLClassifier(configuration: .init())
